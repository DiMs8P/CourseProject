...
namespace CourseProject.Calculus.SlaeSolver
{
    public class MSG
    {
        private int _maxIter = 30000;
        private double _relativeDiscrepancy = 0.0000001;
        ...
        private void MultiplyMatrixByVector(double[] InitialApproximation, double[] Solution)
        {
            ...
            {
                int i0 = _globalMatrix.RowPtr[i - 1];
                int i1 = _globalMatrix.RowPtr[i];
                for (int j = 0; j < (i1 - i0); j++)
                {
                    Solution[i] += _globalMatrix.Values[i0 + j] *
                                   InitialApproximation[_globalMatrix.ColumnPtr[i0 + j]]; // нижний треугольник
                    Solution[_globalMatrix.ColumnPtr[i0 + j]] +=
                        _globalMatrix.Values[i0 + j] * InitialApproximation[i]; // верхний треугольник
                }
            }
        }
